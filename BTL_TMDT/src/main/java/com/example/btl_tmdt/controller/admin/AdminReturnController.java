...
//    }
    ...
    public ResponseEntity<?> acceptReturnOrder(@PathVariable("id") String id) {
        ...
        return ResponseEntity.ok(Map.of(
                "message", "Xác nhận hợp lệ. Tiến hành thanh toán hoàn tiền.",
                "orderId", orderToReturn.getOrderId(),
                "amount", orderToReturn.getTotal()
        ));
    }

    @PostMapping("/confirm-refund/{returnId}")
    public ResponseEntity<?> confirmRefund(@PathVariable("returnId") String id) {
        ReturnOrderDao returnOrder = returnOrderService.getReturnOrderById(id);
        if (returnOrder == null) {
            return ResponseEntity.badRequest().body(Map.of("error", "Không tìm thấy yêu cầu trả hàng."));
        }
        ...
        orderToReturn.setStatus("returned");
        returnOrder.setStatus("accepted");
        System.out.println("Da chap thuan tra hang" + returnOrder + ", trang thai: " + returnOrder.getStatus());
        orderService.updateOrder(orderToReturn);
        returnOrderService.updateReturnOrder(returnOrder);

        return ResponseEntity.ok(Map.of("message", "Đã xác nhận hoàn tiền và cập nhật trạng thái trả hàng."));
    }
